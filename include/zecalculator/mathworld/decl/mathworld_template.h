...
public:
  ...
  using DynMathObject = std::variant<UnregisteredObject, ref<MathObjectType>...>;

  /// @brief const version of the one above
  using ConstDynMathObject = std::variant<UnregisteredObject, cref<MathObjectType>...>;

  /// @brief default constructor that defines the usual functions and global constants
  MathWorldT()
    : MathWorldT(builtin_unary_functions, builtin_binary_functions, builtin_global_constants){};
...
protected:
  ...
  std::tuple<SlottedDeque<MathObjectType>...> math_objects;
